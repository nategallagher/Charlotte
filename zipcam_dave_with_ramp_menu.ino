...
uint16_t shutter_spd_initial = 20;
uint16_t shutter_spd_final = 30000;
uint16_t ramp_start_time = 1;
uint16_t ramp_end_time = 1;
...
float sp = 1;
float shutter_dist = 1;
float shutter_interval = 1;

float run_dist;
uint16_t exposures_remaining;
...
// cleaned up
void recompute() { //recalculate configuration values
    switch (menu) {
        case TOTAL_TIME:
            sp = tot_dist * 1.0 / PREVENT_DIV_ZERO(tot_time);
      ...
      //    exposures = tot_time / shutter_time;
            break;
        case SHUTTER_DIST:
...
String displayTime(uint16_t seconds) {
  ...
  return String("h:") + hour + String(" m:") + minute + String(" s:") + second;
}
  ...
  //  made into switch statement for legibility    -PW 20131125
    switch(menu) {
      ...
      case BULB_RAMP_ENABLE:
	        ...
	        break;
      case SHUTTER_SPD_INITIAL:
  	      lcd.print(shutter_spd_initial);
  	      break;
      case SHUTTER_SPD_FINAL:
  	      lcd.print(shutter_spd_final);
  	      break;
      case RAMP_START_TIME:
